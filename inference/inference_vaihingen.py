...
class VaihingenDataset(PointCloudDataset):

  BaseClass = namedtuple('VaihingenDataset', ['name', 'train_id', 'color'])
...
def get_argparser():
    ...
    parser.add_argument('--threshold',     default= .01,  type=float,   help='minimum confidence threshold')
    ...
    parser.add_argument('--knn_neighbors', default=3,     type=int,     help='interpolate knn neighbors')
...
if __name__ == '__main__':
  ...
  n_features = PointCloudDataset.n_features
  n_classes  = PointCloudDataset.n_classes
  ...
  pointCloudDataset = PointCloudDataset(coords, device=device, n_decimation=decimations, n_neighbors=neighbours)
  ...
  if dense_labels and subsampling:
    ...
    colored_predictions = PointCloudDataset.color_decode_target(predictions)
    dense_coords += mean
